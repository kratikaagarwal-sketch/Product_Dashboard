...
WITH offers AS (
    ...
    FROM im_dwh_rpt.fact_eto_ofr_live
    WHERE eto_ofr_approv_date_orig >= DATE_TRUNC('month', CURRENT_DATE)
...
sales AS (
    ...
    WHERE eto_pur_date >= DATE_TRUNC('month', CURRENT_DATE)
...
blni AS (
    ...
    WHERE eto_ofr_reject_dt >= DATE_TRUNC('month', CURRENT_DATE)
...
export async function GET() {
  try {
    ...
    console.error('Error fetching monthly Redshift data:', error);
